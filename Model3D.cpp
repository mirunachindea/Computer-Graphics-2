...
namespace gps {
	...
	std::vector<glm::vec3> Model3D::getBoundingBox() {
		std::vector<glm::vec3> bbox;
		float xmin = 9999.9f, ymin = 9999.9f, zmin = 9999.9f;
		float xmax = -9999.9f, ymax = -9999.9f, zmax = -9999.9f;

		for (int i = 0; i < meshes.size(); i++) {
			gps::Mesh ctMesh = meshes[i];
			std::vector<Vertex> vertices = ctMesh.vertices;
			for (int j = 0; j < vertices.size(); j++) {
				glm::vec3 v = vertices[j].Position;
		...
		glm::vec3 bboxmin = glm::vec3(xmin, ymin, zmin);
		glm::vec3 bboxmax = glm::vec3(xmax, ymax, zmax);
